/**
 * SceneProps — 章節專屬 3D 場景道具
 *
 * 每個 SceneMode 有獨特的場景道具，讓戰鬥畫面更具沉浸感：
 * - city: 路牌、建築殘骸、街燈、路障
 * - forest: 樹幹、倒木、蘑菇
 * - wasteland: 購物車、破架子、油桶
 * - factory: 機械齒輪、管線架、鐵桶
 * - hospital: 病床、點滴架、醫療櫃
 * - residential: 桌子、椅子、書架
 * - underground: 汽車、交通錐、水泥柱
 * - core: 能量水晶、科技主機、發光管
 *
 * 道具放置使用 deterministic seeded RNG，避免每次重新渲染不同佈局。
 * 中央戰鬥區域 (|x|<7, |z|<9) 不放置道具。
 */
  ...
  return (
    <group position={position} scale={scale}>
      <mesh rotation={[-Math.PI / 2, 0, position[0] * 3.7]} position={[0, 0.005, 0]}>
        ...
        <meshStandardMaterial color="#3a1010" roughness={0.95} transparent opacity={0.6} />
      </mesh>
      <mesh rotation={[-Math.PI / 2, 0, position[2] * 2.1]} position={[0.2, 0.005, 0.15]}>
        <circleGeometry args={[0.2, 5]} />
        <meshStandardMaterial color="#2a0808" roughness={0.95} transparent opacity={0.5} />
      </mesh>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[-0.15, 0.005, -0.1]}>
        <circleGeometry args={[0.15, 6]} />
        <meshStandardMaterial color="#350c0c" roughness={0.95} transparent opacity={0.4} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0, 0.005, 0]} rotation={[-Math.PI / 2 + 0.05, 0, rotation * 2]}>
        ...
        <meshStandardMaterial color="#c8c0a8" roughness={0.9} side={THREE.DoubleSide} />
      </mesh>
      {/* 壓扁罐頭 */}
      ...
      <mesh position={[-0.2, 0.005, 0.1]} rotation={[-Math.PI / 2, 0, rotation * 1.5]}>
        ...
        <meshStandardMaterial color="#aabbcc" roughness={0.1} metalness={0.1} transparent opacity={0.4} />
      </mesh>
    </group>
  )
}
...
function RustMark({ position, normal = [0, 0, 1] as [number, number, number], scale = 1 }: { position: [number, number, number]; normal?: [number, number, number]; scale?: number }) {
  ...
  return (
    <mesh position={position} rotation={rot} scale={scale}>
      ...
      <meshStandardMaterial color="#6a3a18" roughness={0.95} transparent opacity={0.5} side={THREE.DoubleSide} />
    </mesh>
  )
}
  ...
  return (
    <group position={position} rotation={[bent, rotation, 0]}>
      ...
      <mesh position={[0, 1.5, 0]} castShadow>
        ...
        <meshStandardMaterial color="#4a4038" roughness={0.7} metalness={0.4} />
      </mesh>
      {/* 鏽蝕痕跡 */}
      ...
      <mesh position={[0, 2.7, 0.04]}>
        ...
        <meshStandardMaterial color="#6a7a58" roughness={0.7} />
      </mesh>
      {/* 牌面鏽蝕 */}
      <mesh position={[0.2, 2.8, 0.05]} rotation={[0, 0, 0.5]}>
        <circleGeometry args={[0.1, 5]} />
        <meshStandardMaterial color="#5a3a18" roughness={0.95} transparent opacity={0.5} side={THREE.DoubleSide} />
      </mesh>
      {/* 腳下碎石 */}
      ...
      {h > 4 && (
        <>
          ...
          <mesh position={[-w * 0.25, h * 0.55 - 0.22, 0.22]} rotation={[0, 0, 0.1]}>
            ...
            <meshStandardMaterial color="#99aabb" roughness={0.1} transparent opacity={0.3} />
          </mesh>
          <mesh position={[w * 0.25, h * 0.55, 0.21]}>
            <boxGeometry args={[0.4, 0.5, 0.02]} />
            <meshStandardMaterial color="#0a0a12" roughness={0.9} />
  ...
  return (
    <group position={position} rotation={[tilt, rotation, tilt * 0.5]}>
      ...
      <mesh position={[0, 2, 0]} castShadow>
        ...
        <meshStandardMaterial color="#3a3a35" roughness={0.7} metalness={0.4} />
      </mesh>
      {/* 鏽斑 */}
      ...
      <mesh position={[0.4, 3.8, 0]} rotation={[0, 0, -0.5]} castShadow>
        ...
        <meshStandardMaterial color="#3a3a38" roughness={0.6} metalness={0.4} />
      </mesh>
      {/* 破碎燈罩 — 微弱閃爍 */}
      <mesh position={[0.7, 3.7, 0]}>
        <boxGeometry args={[0.3, 0.12, 0.2]} />
        <meshStandardMaterial color="#666650" roughness={0.4} metalness={0.2} emissive="#332200" emissiveIntensity={0.15} />
      </mesh>
      {/* 吊掛電線 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[-0.3, 0.4, 0.26]}>
        ...
        <meshStandardMaterial color="#5a5850" roughness={0.95} transparent opacity={0.5} side={THREE.DoubleSide} />
      </mesh>
      {cracked && (
        <mesh position={[0.2, 0.2, 0.26]} rotation={[0, 0, 0.7]}>
          <boxGeometry args={[0.02, 0.5, 0.01]} />
          <meshStandardMaterial color="#3a3830" roughness={0.95} />
        </mesh>
      )}
      {/* 褪色反光條 */}
      <mesh position={[0, 0.55, 0.26]}>
        <boxGeometry args={[1.6, 0.1, 0.01]} />
        <meshStandardMaterial color="#aa7722" roughness={0.5} emissive="#331100" emissiveIntensity={0.1} />
      </mesh>
      {/* 邊角破損 */}
      <mesh position={[-0.85, 0.6, 0.2]} rotation={[0.3, 0.5, 0.2]}>
        <dodecahedronGeometry args={[0.08, 0]} />
        <meshStandardMaterial color="#7a7870" roughness={0.95} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[-0.2, h * 0.25, 0.15]} rotation={[0.3, 0, -0.5]}>
        <circleGeometry args={[0.1, 5]} />
        <meshStandardMaterial color="#5a7a3a" roughness={0.8} side={THREE.DoubleSide} />
      </mesh>
      <mesh position={[0.18, h * 0.45, -0.12]} rotation={[-0.2, 0.5, 0.3]}>
        <circleGeometry args={[0.07, 5]} />
        <meshStandardMaterial color="#4a6a2a" roughness={0.8} side={THREE.DoubleSide} />
      </mesh>
      {/* 爪痕 */}
      {h > 3.5 && (
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0, 0.4, 0.1]}>
        ...
        <meshStandardMaterial color="#2a5a20" roughness={0.9} />
      </mesh>
      <mesh position={[-0.4, 0.42, -0.08]}>
        <circleGeometry args={[0.2, 5]} />
        <meshStandardMaterial color="#1a4a15" roughness={0.9} side={THREE.DoubleSide} />
      </mesh>
      {/* 斷裂端面 */}
      <mesh position={[1.25, 0.25, 0]} rotation={[0, 0, Math.PI / 2]}>
        <circleGeometry args={[0.2, 7]} />
        <meshStandardMaterial color="#5a4028" roughness={0.95} side={THREE.DoubleSide} />
      </mesh>
      {/* 小蘑菇叢 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]} scale={s}>
      ...
      <mesh position={[0, 0.65, 0]} castShadow>
        ...
        <meshStandardMaterial color="#8b2020" roughness={0.7} emissive={glowColor} emissiveIntensity={0.15} />
      </mesh>
      {/* 斑點 */}
      <mesh position={[0.08, 0.7, 0.08]}>
        <sphereGeometry args={[0.04, 4, 4]} />
        <meshStandardMaterial color="#ddddcc" roughness={0.6} />
      </mesh>
      <mesh position={[-0.06, 0.68, -0.1]}>
        <sphereGeometry args={[0.03, 4, 4]} />
        <meshStandardMaterial color="#ccccbb" roughness={0.6} />
      </mesh>
      {/* 菌絲蔓延地面 */}
      <mesh position={[0, 0.005, 0]} rotation={[-Math.PI / 2, 0, rotation]}>
        <circleGeometry args={[0.3, 6]} />
        <meshStandardMaterial color="#3a4a2a" roughness={0.95} transparent opacity={0.35} side={THREE.DoubleSide} />
      </mesh>
      {/* 旁邊小菇 */}
      <mesh position={[0.2, 0.12, 0.1]}>
        <cylinderGeometry args={[0.02, 0.03, 0.24, 4]} />
        <meshStandardMaterial color="#c8b888" roughness={0.8} />
      </mesh>
      <mesh position={[0.2, 0.26, 0.1]}>
        <sphereGeometry args={[0.06, 6, 4, 0, Math.PI * 2, 0, Math.PI / 2]} />
        <meshStandardMaterial color="#7a1818" roughness={0.7} emissive={glowColor} emissiveIntensity={0.1} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[(Math.sin(rotation * 3) - 0.5) * 0.3, rotation, 0]}>
      ...
      <mesh position={[0, 0.55, 0]} castShadow>
        ...
        <meshStandardMaterial color="#7a7068" roughness={0.6} metalness={0.4} wireframe />
      </mesh>
      {/* 車籃底板 */}
      <mesh position={[0, 0.36, 0]}>
        <boxGeometry args={[0.58, 0.02, 0.43]} />
        <meshStandardMaterial color="#6a6058" roughness={0.7} metalness={0.4} />
      </mesh>
      {/* 把手 */}
      <mesh position={[0, 0.8, -0.22]} castShadow>
        <boxGeometry args={[0.5, 0.04, 0.04]} />
        <meshStandardMaterial color="#5a5550" roughness={0.6} metalness={0.4} />
      </mesh>
      {/* 輪子 — 部分缺失 */}
      {[[-0.22, 0.08, 0.15], [0.22, 0.08, 0.15], [-0.22, 0.08, -0.15]].map((p, i) => (
        <mesh key={i} position={p as [number, number, number]} rotation={[Math.PI / 2, 0, 0]}>
          <cylinderGeometry args={[0.06, 0.06, 0.03, 8]} />
          <meshStandardMaterial color="#333330" roughness={0.6} metalness={0.3} />
        </mesh>
      ))}
      {/* 鏽斑 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, (Math.sin(rotation * 7) - 0.5) * 0.15]}>
      ...
      <mesh position={[-0.3, 0.8, 0.21]} rotation={[0, 0, 0.3]}>
        <circleGeometry args={[0.12, 5]} />
        <meshStandardMaterial color="#5a4838" roughness={0.95} transparent opacity={0.4} side={THREE.DoubleSide} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[tipped ? 1.4 : 0, rotation, 0]}>
      <mesh position={[0, tipped ? 0.28 : 0.5, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#8a5518" roughness={0.7} metalness={0.3} />
      </mesh>
      {/* 桶蓋 */}
      <mesh position={[0, tipped ? 0.73 : 0.95, 0]}>
        <cylinderGeometry args={[0.26, 0.26, 0.04, 10]} />
        <meshStandardMaterial color="#7a5518" roughness={0.6} metalness={0.4} />
      </mesh>
      {/* 鏽帶 */}
      <mesh position={[0, tipped ? 0.15 : 0.35, 0]}>
        <cylinderGeometry args={[0.29, 0.29, 0.06, 10]} />
        <meshStandardMaterial color="#5a3010" roughness={0.95} metalness={0.2} />
      </mesh>
      ...
      {leaking && tipped && (
        <mesh position={[0.4, 0.005, 0]} rotation={[-Math.PI / 2, 0, rotation]}>
          <circleGeometry args={[0.3, 6]} />
          <meshStandardMaterial color="#1a1508" roughness={0.95} transparent opacity={0.5} />
        </mesh>
      )}
      {/* 標籤殘留 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]} scale={s}>
      ...
      <mesh position={[0, 0.8, 0]} rotation={[Math.PI / 2, 0, 0]} castShadow>
        ...
        <meshStandardMaterial color="#454550" roughness={0.5} metalness={0.6} />
      </mesh>
      {/* 中心軸 */}
      <mesh position={[0, 0.8, 0]} rotation={[Math.PI / 2, 0, 0]} castShadow>
        <cylinderGeometry args={[0.15, 0.15, 0.3, 8]} />
        <meshStandardMaterial color="#555560" roughness={0.3} metalness={0.7} />
      </mesh>
      {/* 支撐座 */}
      <mesh position={[0, 0.3, 0]} castShadow receiveShadow>
        <boxGeometry args={[0.8, 0.6, 0.5]} />
        <meshStandardMaterial color="#3a3a42" roughness={0.8} metalness={0.3} />
      </mesh>
      {/* 鏽蝕大型斑塊 */}
      ...
      <mesh position={[0, 0.005, 0.2]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial color="#1a1a10" roughness={0.95} transparent opacity={0.4} />
      </mesh>
      {/* 底部碎屑 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0, 1.2, 0]} rotation={[0, 0, Math.PI / 2]} castShadow>
        ...
        <meshStandardMaterial color="#7a6050" roughness={0.6} metalness={0.4} />
      </mesh>
      <mesh position={[0, 0.8, 0.3]} rotation={[0, 0, Math.PI / 2]} castShadow>
        <cylinderGeometry args={[0.08, 0.08, 2.2, 7]} />
        <meshStandardMaterial color="#8a6e38" roughness={0.7} metalness={0.3} />
      </mesh>
      {/* 支架 */}
      {[-0.9, 0.9].map(x => (
        <mesh key={x} position={[x, 0.6, 0.15]} castShadow>
          <boxGeometry args={[0.08, 1.2, 0.08]} />
          <meshStandardMaterial color="#4a4a52" roughness={0.6} metalness={0.5} />
        </mesh>
      ))}
      {/* 管子接口漏水 */}
      <mesh position={[0.5, 0.9, 0.15]} rotation={[-Math.PI / 2 + 0.2, 0, 0]}>
        <circleGeometry args={[0.08, 5]} />
        <meshStandardMaterial color="#3a5a4a" roughness={0.9} transparent opacity={0.5} side={THREE.DoubleSide} />
      </mesh>
      {/* 鏽印 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0, 0.45, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#484850" roughness={0.6} metalness={0.5} />
      </mesh>
      {/* 腳架 */}
      {[[-1, 0.22, 0.3], [1, 0.22, 0.3], [-1, 0.22, -0.3], [1, 0.22, -0.3]].map((p, i) => (
        <mesh key={i} position={p as [number, number, number]} castShadow>
          <boxGeometry args={[0.08, 0.44, 0.08]} />
          <meshStandardMaterial color="#3a3a42" roughness={0.6} metalness={0.5} />
        </mesh>
      ))}
      {/* 滾輪 */}
      {[-0.8, 0, 0.8].map(x => (
        <mesh key={x} position={[x, 0.5, 0]} rotation={[0, 0, Math.PI / 2]} castShadow>
          ...
          <meshStandardMaterial color="#5a5a62" roughness={0.4} metalness={0.6} />
        </mesh>
      ))}
      {/* 斷裂傳送帶殘片 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0.15, 0.565, 0.2]} rotation={[-Math.PI / 2, 0, 0.3]}>
        ...
        <meshStandardMaterial color="#4a1515" roughness={0.95} transparent opacity={0.5} />
      </mesh>
      {/* 床架 */}
      <mesh position={[0, 0.38, 0]}>
        <boxGeometry args={[0.92, 0.06, 2.02]} />
        <meshStandardMaterial color="#8a99aa" roughness={0.5} metalness={0.4} />
      </mesh>
      {/* 腳 */}
      ...
      <mesh position={[0, 0.75, -0.95]} castShadow>
        ...
        <meshStandardMaterial color="#7a8899" roughness={0.5} metalness={0.4} />
      </mesh>
      {/* 枕頭 — 歪斜 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, (Math.sin(rotation * 9) - 0.5) * 0.2]}>
      ...
      <mesh position={[0, 1.82, 0]}>
        ...
        <meshStandardMaterial color="#999999" roughness={0.4} metalness={0.5} />
      </mesh>
      {/* 點滴袋 */}
      <mesh position={[0.08, 1.6, 0]}>
        <boxGeometry args={[0.1, 0.2, 0.06]} />
        <meshStandardMaterial color="#8aaa99" roughness={0.6} transparent opacity={0.6} />
      </mesh>
      {/* 輸液管 — 垂下 */}
      ...
      <mesh position={[0.1, 0.005, 0.05]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial color="#3a5a4a" roughness={0.95} transparent opacity={0.35} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[doorOpen ? 0.25 : 0, 0.6, doorOpen ? 0.25 : 0.18]} rotation={[0, doorOpen ? 0.8 : 0, 0]}>
        ...
        <meshStandardMaterial color="#bbc0bb" roughness={0.6} />
      </mesh>
      {/* 把手 */}
      <mesh position={[doorOpen ? 0.05 : 0.2, 0.6, doorOpen ? 0.35 : 0.2]}>
        <boxGeometry args={[0.04, 0.12, 0.03]} />
        <meshStandardMaterial color="#999999" roughness={0.4} metalness={0.4} />
      </mesh>
      {/* 紅十字 — 褮色 */}
      ...
      {doorOpen && (
        <>
          <mesh position={[-0.1, 0.45, 0.05]}>
            ...
            <meshStandardMaterial color="#cc7733" roughness={0.5} />
          </mesh>
          <mesh position={[0.1, 0.85, 0.0]}>
            <cylinderGeometry args={[0.025, 0.025, 0.1, 5]} />
            <meshStandardMaterial color="#33aa55" roughness={0.5} />
          </mesh>
        </>
      )}
      {/* 櫃子下方地面散落藥瓶 */}
      <mesh position={[0.2, 0.02, 0.25]} rotation={[Math.PI / 2, 0, rotation * 2]}>
        ...
        <meshStandardMaterial color="#ddaa44" roughness={0.5} />
      </mesh>
      {/* 汙漬 */}
      <mesh position={[-0.15, 0.3, 0.19]} rotation={[0, 0, 0.5]}>
        <circleGeometry args={[0.08, 5]} />
        <meshStandardMaterial color="#6a5a4a" roughness={0.95} transparent opacity={0.4} side={THREE.DoubleSide} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0.15, 0.685, 0.05]} rotation={[-Math.PI / 2, 0, 0.6]}>
        <circleGeometry args={[0.1, 5]} />
        <meshStandardMaterial color="#4a3015" roughness={0.95} transparent opacity={0.35} />
      </mesh>
      {/* 桌上散落物 — 破盤子 */}
      <mesh position={[-0.2, 0.7, 0.1]} rotation={[-Math.PI / 2 + 0.03, 0, rotation]}>
        <circleGeometry args={[0.1, 7]} />
        <meshStandardMaterial color="#c8c0b0" roughness={0.7} side={THREE.DoubleSide} />
      </mesh>
      {/* 桌下垃圾 */}
  ...
  return (
    <group position={position} rotation={[tipped ? 1.2 : 0, rotation, 0]}>
      ...
      <mesh position={[0, 0.4, 0]} castShadow>
        ...
        <meshStandardMaterial color="#7a5828" roughness={0.85} />
      </mesh>
      {/* 椅背 — 有封壞裂縫 */}
      ...
      {[[-0.16, 0.2, 0.16], [0.16, 0.2, 0.16], [-0.16, 0.18, -0.16], [0.16, 0.2, -0.16]].map((p, i) => (
        <mesh key={i} position={p as [number, number, number]}>
          ...
          <meshStandardMaterial color="#4a2808" roughness={0.85} />
        </mesh>
      ))}
    </group>
  )
  ...
  return (
    <group position={position} rotation={[0, rotation, (Math.sin(rotation * 5) - 0.5) * 0.1]}>
      ...
      <mesh position={[0, 0.9, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#5a4018" roughness={0.85} />
      </mesh>
      {/* 層板 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0, 0.5, 0]} castShadow>
        ...
        <meshStandardMaterial color="#2a2a32" roughness={0.6} metalness={0.2} />
      </mesh>
      {/* 螢幕 (碎裂) */}
      <mesh position={[0, 0.5, 0.045]}>
        <boxGeometry args={[0.6, 0.36, 0.01]} />
        <meshStandardMaterial color="#1a1a28" roughness={0.3} metalness={0.1} />
      </mesh>
      {/* 碎裂紋 */}
      {cracked && (
        <>
          <mesh position={[-0.05, 0.55, 0.052]} rotation={[0, 0, 0.7]}>
            ...
            <meshStandardMaterial color="#444450" roughness={0.2} />
          </mesh>
          ...
          <mesh position={[0, 0.5, 0.053]}>
            ...
            <meshStandardMaterial color="#667788" roughness={0.1} transparent opacity={0.3} />
          </mesh>
        </>
      )}
      {/* 底座 */}
      <mesh position={[0, 0.22, 0]}>
        ...
        <meshStandardMaterial color="#333340" roughness={0.5} metalness={0.3} />
      </mesh>
      {/* 灰塵 */}
      <mesh position={[0, 0.74, 0.01]} rotation={[-Math.PI / 2, 0, 0]}>
        <boxGeometry args={[0.65, 0.06, 0.005]} />
        <meshStandardMaterial color="#7a7868" roughness={0.95} transparent opacity={0.3} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[(Math.sin(rotation * 3) - 0.5) * 0.08, rotation, 0]}>
      ...
      <mesh position={[0, 0.45, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color={burned ? '#2a2020' : '#3a4050'} roughness={0.85} metalness={0.2} />
      </mesh>
      {/* 車頂/駕駛艙 — 凹陷 */}
      <mesh position={[0.1, burned ? 0.85 : 0.9, 0]} castShadow>
        <boxGeometry args={[0.9, burned ? 0.38 : 0.45, 0.8]} />
        <meshStandardMaterial color={burned ? '#1a1818' : '#2a3040'} roughness={0.7} metalness={0.15} />
      </mesh>
      {/* 車窗 — 破碎 */}
      <mesh position={[0.1, 0.92, 0.41]}>
        <boxGeometry args={[0.5, 0.2, 0.01]} />
        <meshStandardMaterial color="#445566" roughness={0.2} metalness={0.1} transparent opacity={0.35} />
      </mesh>
      {/* 碎玻璃散落 */}
      {[[-0.2, 0.02, 0.55], [0.3, 0.02, 0.6]].map((p, i) => (
        <mesh key={`g${i}`} position={p as [number, number, number]} rotation={[-Math.PI / 2, 0, rotation * (i + 1)]}>
          ...
          <meshStandardMaterial color="#88aacc" roughness={0.1} transparent opacity={0.3} />
        </mesh>
      ))}
      {/* 輪子 — 缺一個 */}
      {[[-0.55, 0.18, 0.45], [0.55, 0.18, 0.45], [-0.55, 0.18, -0.45]].map((p, i) => (
        <mesh key={i} position={p as [number, number, number]} rotation={[Math.PI / 2, 0, 0]}>
          ...
          <meshStandardMaterial color="#222228" roughness={0.8} />
        </mesh>
      ))}
      {/* 車頭燈 (碎裂) */}
      <mesh position={[-0.75, 0.45, 0.3]}>
        <sphereGeometry args={[0.08, 6, 6]} />
        <meshStandardMaterial color="#998866" roughness={0.3} metalness={0.2} />
      </mesh>
      {/* 鏽斑 */}
      ...
      <mesh position={[-0.3, 0.01, -0.3]} rotation={[-Math.PI / 2, 0, 0]}>
        <circleGeometry args={[0.25, 6]} />
        <meshStandardMaterial color="#1a1a10" roughness={0.95} transparent opacity={0.35} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[fallen ? 1.4 : 0, rotation, 0]}>
      <mesh position={[0, fallen ? 0.15 : 0.25, 0]} castShadow>
        ...
        <meshStandardMaterial color="#aa4400" roughness={0.8} />
      </mesh>
      {/* 反光帶 — 褪色 */}
      <mesh position={[0, fallen ? 0.22 : 0.32, 0]}>
        <coneGeometry args={[0.12, 0.1, 6]} />
        <meshStandardMaterial color="#ccccaa" roughness={0.5} />
      </mesh>
      {/* 底座 */}
      <mesh position={[0, fallen ? 0.02 : 0.03, 0]}>
        <boxGeometry args={[0.3, 0.04, 0.3]} />
        <meshStandardMaterial color="#993c00" roughness={0.8} />
      </mesh>
      {/* 髒污 */}
      <mesh position={[0.02, fallen ? 0.1 : 0.15, 0.1]} rotation={[0.3, 0, 0.1]}>
        <boxGeometry args={[0.08, 0.15, 0.005]} />
        <meshStandardMaterial color="#5a4020" roughness={0.95} transparent opacity={0.3} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      <mesh position={[0, h / 2, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#585e68" roughness={0.88} />
      </mesh>
      {/* 裂縫紋路 — 多條 */}
      ...
      <mesh position={[-0.2, h * 0.6, 0.2]} rotation={[0, 0.4, -0.15]}>
        ...
        <meshStandardMaterial color="#404448" roughness={0.9} />
      </mesh>
      {/* 鋼筋外露 */}
      <mesh position={[0.28, h * 0.7, 0.1]} rotation={[0, 0, 0.1]}>
        <cylinderGeometry args={[0.015, 0.015, 0.5, 4]} />
        <meshStandardMaterial color="#8a5030" roughness={0.7} metalness={0.4} />
      </mesh>
      {/* 底部水漬 */}
      <mesh position={[0, 0.01, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        <circleGeometry args={[0.5, 8]} />
        <meshStandardMaterial color="#4a5058" roughness={0.95} transparent opacity={0.2} />
      </mesh>
      {/* 銹斑 */}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[bent ? -0.1 : 0, bent ? 0.5 : 0.6, 0]} rotation={[0, 0, Math.PI / 2 + (bent ? 0.15 : 0)]} castShadow>
        ...
        <meshStandardMaterial color="#bbaa18" roughness={0.65} metalness={0.25} />
      </mesh>
      {/* 立柱 */}
      {[-0.9, 0.9].map(x => (
  ...
  return (
    <group ref={ref} position={position} scale={s}>
      ...
      <mesh position={[0.5, 0.3, 0]}>
        ...
        <meshStandardMaterial color="#cc66ee" emissive="#aa44cc" emissiveIntensity={1} transparent opacity={0.7} />
      </mesh>
      <mesh position={[-0.35, -0.2, 0.3]}>
        <octahedronGeometry args={[0.1, 0]} />
        <meshStandardMaterial color="#bb55dd" emissive="#9933bb" emissiveIntensity={0.8} transparent opacity={0.6} />
      </mesh>
      {/* 底座光環 */}
      <mesh position={[0, -0.4, 0]} rotation={[Math.PI / 2, 0, 0]}>
        <torusGeometry args={[0.4, 0.03, 6, 16]} />
        <meshStandardMaterial color="#884488" emissive="#662288" emissiveIntensity={2} transparent opacity={0.5} />
      </mesh>
      {/* 地面裂縫—能量侵蝕 */}
      <mesh position={[0, -0.48, 0]} rotation={[-Math.PI / 2, 0, rotation * 3]}>
        <ringGeometry args={[0.5, 0.8, 6]} />
        <meshStandardMaterial color="#6622aa" emissive="#4411aa" emissiveIntensity={0.6} transparent opacity={0.2} side={THREE.DoubleSide} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <group position={position} rotation={[0, rotation, 0]}>
      ...
      <mesh position={[0, 0.5, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#2a1830" roughness={0.6} metalness={0.3} />
      </mesh>
      {/* 螢幕 — 碎裂/正常 */}
      <mesh position={[0, 0.7, 0.26]}>
        ...
        <meshStandardMaterial
          ...
          emissive={damaged ? '#220033' : '#440088'}
          emissiveIntensity={damaged ? 0.3 : 0.8}
        />
      </mesh>
      {/* 碎裂紋 */}
      {damaged && (
        <mesh position={[-0.08, 0.75, 0.275]} rotation={[0, 0, 0.5]}>
          ...
          <meshStandardMaterial color="#443355" roughness={0.2} />
        </mesh>
      )}
      {/* 指示燈 */}
      {[[-0.25, 0.35], [0, 0.35], [0.25, 0.35]].map(([x, y], i) => (
        <mesh key={i} position={[x, y, 0.26]}>
          ...
          <meshStandardMaterial
            ...
            emissive={i === 0 ? '#22aa22' : i === 1 ? '#aa6600' : '#aa0000'}
            emissiveIntensity={damaged && i !== 2 ? 0.5 : 2}
          />
        </mesh>
      ))}
      {/* 地面電線 */}
    ...
    <group position={position} rotation={[0, rotation, broken ? 0.2 : 0]}>
      <mesh position={[0, h / 2, 0]} castShadow>
        ...
        <meshStandardMaterial
          ...
          emissive={broken ? '#221155' : '#4422aa'}
          emissiveIntensity={broken ? 0.5 : 2}
          transparent
      ...
      <mesh position={[0, 0.05, 0]}>
        ...
        <meshStandardMaterial color="#3a2048" roughness={0.7} metalness={0.3} />
      </mesh>
      {/* 頂座 */}
      ...
      <mesh position={[0.05, 0.01, 0.1]} rotation={[-Math.PI / 2, 0, 0]}>
        <circleGeometry args={[0.06, 5]} />
        <meshStandardMaterial color="#5522aa" emissive="#3311aa" emissiveIntensity={0.8} transparent opacity={0.25} />
      </mesh>
    </group>
  )
}
...
function generateSceneElements(mode: SceneMode, stageId: string = '1-1'): React.ReactNode[] {
  ...
  switch (mode) {
    ...
    // tower / daily / pvp / boss → 不額外加道具（使用原本 debris）
    default:
      break
  }
  return elements
