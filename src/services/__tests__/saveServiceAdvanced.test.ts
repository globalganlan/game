...
describe('saveService — getTimerYield', () => {
  it('stageId "1-1" → 最低產出', () => {
    ...
    expect(y.expPerHour).toBe(Math.max(100, 1 * 50)) // 100
  })
  ...
  it('stageId "1-8" → chapter 1 最後一關', () => {
    ...
    expect(y.expPerHour).toBe(Math.max(100, 8 * 50)) // 400
  })
  ...
  it('stageId "3-5" → 中段進度', () => {
    ...
    expect(y.expPerHour).toBe(Math.max(100, 21 * 50)) // 1050
  })
  ...
  it('stageId "3-8" → 最高進度', () => {
    ...
    expect(y.expPerHour).toBe(Math.max(100, 24 * 50)) // 1200
  })
...
describe('saveService — getAccumulatedResources', () => {
  ...
  it('1 小時累積 (stageId = "1-1")', () => {
    ...
    expect(result.exp).toBe(100) // 100 exp/h × 1h (max(100, 50))
  ...
  it('3 小時累積 (stageId = "2-5")', () => {
    ...
    const { goldPerHour, expPerHour } = getTimerYield('2-5')
    expect(result.gold).toBe(Math.floor(goldPerHour * 3))
    expect(result.exp).toBe(Math.floor(expPerHour * 3))
  })
