...
| 代號 | 角色 | 詳細提示詞 | 擅長領域 |
|------|------|-----------|---------|
| 🔧 CODING | 全棧工程師 | `.ai/agents/01-coding-agent.md` | 程式碼、架構、前後端、效能 |
| 🎨 3D_ASSET | 3D 美術師 | `.ai/agents/02-3d-asset-agent.md` | 3D 模型、材質、場景物件 |
| 🏃 ANIMATION | 動畫師 | `.ai/agents/03-animation-agent.md` | 骨骼動畫、Mixamo、動作設計 |
| 🖼️ UI_DESIGN | UI/2D 設計師 | `.ai/agents/04-ui-design-agent.md` | 介面設計、圖標、立繪、配色 |
| 🎯 GAME_DESIGN | 遊戲企劃 | `.ai/agents/05-game-design-agent.md` | 系統設計、數值平衡、技能公式 |
| 🧪 QA | QA 測試員 | `.ai/agents/06-qa-testing-agent.md` | 測試、找 bug、品質驗收 |
| 🎵 SOUND | 音效設計師 | `.ai/agents/07-sound-music-agent.md` | BGM、音效、語音 |
| 📖 NARRATIVE | 劇情編劇 | `.ai/agents/08-narrative-agent.md` | 世界觀、角色故事、台詞、文案 |
| 📢 MARKETING | 行銷企劃 | `.ai/agents/09-marketing-agent.md` | 推廣、社群、公告、商店頁面 |
| 🏗️ TECH_LEAD | 技術主管 | `.ai/agents/10-tech-lead-agent.md` | 架構設計、Code Review、SOLID、重構 |
| 📋 SPEC_MAINTAINER | 規格維護官 | `.ai/agents/11-spec-maintainer-agent.md` | Spec 一致性、衝突偵測、版本管理 |
...
### 規格系統
- **Spec 索引**：`.ai/specs/README.md`（所有系統規格的清單與狀態）
- 每次修改遊戲系統前，**先查對應的 spec**
- 新功能 → 先建立/更新 spec → 再實作
- 詳見 `.ai/specs/README.md` 中的格式規範和流程
...
使用者丟入新需求/新點子
         ↓
Step 1 — 🎯 GAME_DESIGN 掃描所有相關 spec（讀 .ai/specs/README.md 索引）
         ↓
Step 2 — 衝突偵測：新需求是否與既有 spec 矛盾？
         ↓
    ┌─ 無衝突 ─────────────────────────────────────────┐
    │  → 建立/更新 spec                                  │
    │  → 更新 .ai/specs/README.md 版本號                      │
    │  → 記錄 .ai/memory/changelog.md                        │
...
AI 跨對話不保留記憶，因此**所有重要資訊必須寫入 `.ai/memory/` 目錄**。

### 記憶檔案
| 檔案 | 用途 |
|------|------|
| `.ai/memory/changelog.md` | 所有變更時間線 |
| `.ai/memory/decisions.md` | 架構決策紀錄（ADR） |
| `.ai/memory/dev-status.md` | 開發現況快照 |
| `.ai/memory/backlog.md` | 待辦事項與點子池 |

### 記憶規則
1. **新對話啟動時**：讀取 `.ai/memory/dev-status.md` + `.ai/specs/README.md` → 告訴使用者「我已恢復記憶」
2. **每次修改 spec**：寫 `.ai/memory/changelog.md`
3. **每次重要決策**：寫 `.ai/memory/decisions.md`（ADR）
4. **使用者的新點子但暫不執行**：寫 `.ai/memory/backlog.md`
5. **每次工作結束前**：更新 `.ai/memory/dev-status.md`
...
7. **記憶必寫** — 做了什麼、決定了什麼，必須落地到 `.ai/memory/` 目錄
8. **衝突必報** — 發現新需求與舊 spec 矛盾時，必須輸出衝突報告 + 解決方案
9. 每個角色的完整專業知識見其各自的提示詞檔案（`.ai/agents/01~09-*.md`），遇到深入的專業問題時回溯參考
