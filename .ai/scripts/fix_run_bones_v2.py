...
def get_target_bones(model_dir, name):
    """Import idle.glb temporarily to get generic bone names and prefix."""
...
def process(num):
    ...
    # 3. Rename bones on armature (updates fcurve data_paths automatically)
    if src_prefix != target_prefix:
        print(f"  Renaming: {src_prefix} -> {target_prefix}")
    ...
    print(f"  After rename: {len(renamed_generic)} bones")
    ...
    for fc in bag.fcurves:
        ...
        if m:
            final.add(m.group(2))
    print(f"  Final: {len(final)} anim bones, {len(armature.data.bones)} skel")
