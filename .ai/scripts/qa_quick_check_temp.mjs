...
await p.goto('http://localhost:5177/game/', { waitUntil: 'networkidle2', timeout: 60000 })
...
for (const btn of await p.$$('button')) {
  ...
  if (txt && (txt.includes('?脣') || txt.includes('??') || txt.includes('?') || txt.includes('Play'))) {
...
while (Date.now() < deadline) {
  ...
  if (txt.includes('??脣漲') || txt.includes('?砍?') || txt.includes('??')) {
    foundMenu = true
    break
  }
...
console.log('--- END ---')
const match = text.match(/??脣漲[嚗?]\s*(\S+)/)
console.log('??脣漲:', match ? match[1] : 'NOT FOUND in page')
...
const gi = btns.findIndex(t => t && t.includes('?砍?'))
if (gi >= 0) {
  ...
  if (sb) {
    ...
    console.log('gacha pull:', has ? '??OK' : dis ? '?? disabled (no diamond)' : '??FAIL')
  }
} else {
