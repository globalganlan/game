"""
將 rebind_mixamo_anims.mjs 下載的 FBX 轉為動畫 GLB
只處理 7 個重綁模型，輸出格式: zombie_X_{anim}.glb

用法：
  D:\Blender\blender.exe --background --python .ai/scripts/convert_rebind_fbx.py
"""
import bpy
import os
import sys
...
TARGETS = [
    "zombie_16", "zombie_17", "zombie_19",
    "zombie_21", "zombie_25", "zombie_26", "zombie_28",
]
...
print(f"\n{'='*60}")
print("FBX → GLB 動畫轉換（7 個重綁模型）")
print(f"{'='*60}\n")

for zombie_id in TARGETS:
    ...
    for anim in ANIM_NAMES:
        ...
        if not os.path.exists(fbx_path):
            print(f"  ⚠ {zombie_id}/{anim}.fbx 不存在，跳過")
            continue

        # 輸出: zombie_17_idle.glb
        glb_path = os.path.join(zombie_dir, f"{zombie_id}_{anim}.glb")
        # 備份舊 GLB
        if os.path.exists(glb_path):
            bak_dir = os.path.join(zombie_dir, "bak_mixamo_rebind")
