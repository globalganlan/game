...
| 檔案 | 說明 |
|------|------|
| `.ai/scripts/fbx_to_glb.py` | Blender Python 主腳本 — FBX 匯入、Decimate、貼圖縮放、GLB 匯出 |
| `.ai/scripts/convert_models.ps1` | PowerShell 包裝器 — 自動偵測 Blender 路徑並呼叫主腳本 |
...
# 方法二：直接呼叫 Blender
& "D:\Blender\blender.exe" --background --python .ai/scripts/fbx_to_glb.py
```
...
# 直接呼叫
& "D:\Blender\blender.exe" --background --python .ai/scripts/fbx_to_glb.py -- --only zombie_6
```
