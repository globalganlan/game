...
| `inventory` | `InventoryPanel` | 背包面板（道具分類、詳情、使用/裝備/分解） | 通關 1-1 後 |
...
| `arena` | `ArenaPanel` | 競技場面板（排名、防守陣型、挑戰、獎勵） | 通關 1-5 後 |
...
5. **錯誤訊息**（若有）
6. **抽卡按鈕**（三按鈕布局）：免費單抽或抽卡券（最左）/ 單抽（160 鑽）/ 十連抽（1,600 鑽）
...
- 2 個分類 Tab：道具（所有非裝備道具）/ 裝備（含裝備列表 + 裝備圖鑑子介面）
- 道具列表 + 詳情檢視 + 使用操作
...
- **Boss 傷害條**（BossDamageBar，Boss 模式時顯示累計傷害 + 段位門檻 + 回合計數「回合 N/M」）
...
| 背包 | InventoryPanel.tsx | `<CurrencyIcon>` for Header 貨幣列 |
...
**Workers Handler**：`handleDailyCheckin_()`
...
**Service**：`saveService.ts` — `doDailyCheckin()` 呼叫 Workers daily-checkin API
...
- **佈局限制**：`max-height: 28vh` + `overflow: hidden`，防止內容過多時遮擋敵方陣型（尤其 Boss/競技場多行獎勵時）
...
3. 清除所有戰鬥狀態（turn、popups、battleResult、actorStates、buffs、energy、skillToasts、passiveHints 等）
...
| v2.8 | 2026-03-09 | **戰鬥準備 banner 壓縮佈局**：`battle-prep-top-banner` 加入 `max-height: 28vh` + `overflow: hidden` 限制高度，縮小 gap/padding/字體（gap 4→2px, padding 8→4px, reward-row font 0.7→0.62rem, stage-header font 0.8→0.72rem），`cp-comparison` 壓縮 padding 6→3px，防止 Boss/競技場多行獎勵時遮擋敵方陣型 |
