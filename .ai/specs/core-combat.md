...
| v4.3 | 2026-03-16 | **KOF98 風格全螢幕白閃**：終結技施放時新增 2D CSS overlay 全螢幕三連白閃效果（仿格鬥天王98 超必殺技閃光）。`useAnimationPromises.ts` 增加 `skillFlashOverlayKey` state，`addSkillFlash` 同時遞增 key 觸發 overlay。`App.tsx` 在 Canvas 外層渲染 `.kof-flash-overlay` div（`z-index:9000, pointer-events:none`），使用 `@keyframes kof-flash` 0.35s 三次白閃動畫（0%→0.95→0→0.85→0→0.65→0）。`onAnimationEnd` 自動隱藏元素 |