...
| v4.2 | 2026-03-14 | **被動文字全面守衛 + 終結技閃光可見化**：(1) `every_n_turns` 內聯被動處理新增 `anyEffectApplied` 檢查，未生效不 emit `PASSIVE_TRIGGER`；(2) `checkHpBelowPassives` 同樣新增 `anyEffectApplied` 檢查；Workers 端同步；(3) `SkillFlash` 從單純 PointLight 改為 PointLight + AdditiveBlending 發光球體（sphere r=2.5, opacity peak 0.45, 0.5s 持續），確保大招閃光肉眼可見 |
| v4.3 | 2026-03-16 | **KOF2002 四射光芒終結技閃光**：(1) 移除 v4.3 的全螢幕 CSS overlay（`kof-flash-overlay` + `@keyframes kof-flash`）及 `skillFlashOverlayKey` state；(2) `SkillFlash` 從球體改為 KOF2002 風格四射光芒 — `makeBurstGeometry()` 生成 16 條隨機方向尖銳三角形射線（innerR=0.15, outerR=2.8, halfWidth=0.08），球面分佈偏水平展開；AdditiveBlending + DoubleSide + depthTest:false；0.45s 快速爆發衰減動畫（peak opacity 0.9）；PointLight distance=12 配合；Hero.tsx 改用白色 `#ffffff` intensity=12 |