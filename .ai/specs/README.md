...
| 檔案 | 系統 | 版本 | 狀態 |
|------|------|------|------|
| `.ai/specs/core-combat.md` | 回合制戰鬥系統（Domain Engine + Command Pattern） | v4.0 | 🟢 已實作 |
| `.ai/specs/hero-schema.md` | 英雄資料結構（三層型別：RawHeroData → RawHeroInput → BattleHero + HeroInstance.stars） | v2.6 | 🟢 已實作 |
| `.ai/specs/damage-formula.md` | 傷害公式（11 步流程 0-10：閃避→DEF→暴擊→護盾→反彈） | v1.3 | 🟢 已實作 |
| `.ai/specs/skill-system.md` | 技能系統（SkillTemplate + SkillEffect + Sheets 資料管線） | v1.5 | 🟢 已實作 |
...
| `.ai/specs/gacha.md` | 抽卡系統（本地池架構 + 裝備抽卡 v2） | v2.4 | 🟢 已實作 |
...
| `.ai/specs/ui-flow.md` | UI 流程與畫面定義（GameState/MenuScreen 10 值/導航函式/條件式過場幕/CurrencyIcon） | v3.0 | 🟢 已實作 |
...
| `.ai/specs/combat-power.md` | 戰力系統（六維加權公式 + HUD 顯示 + 敵我對比 + 戰力變化動畫） | v0.1 | 🟢 已實作 |
| `.ai/specs/arena-pvp.md` | 競技場排名（500 名制 + 防守陣型 + NPC 佔位 + 每週重置 + 4 層獎勵） | v0.1 | 🟢 已實作 |
| `.ai/specs/item-acquire-toast.md` | 獲得物品動畫提示（5 類物品 + 稀有度光效 + 佇列播放 + 合併列表 + 跳過機制） | v0.1 | 🟢 已實作 |
...
| `src/domain/combatPower.ts` | combat-power | 戰力計算公式（六維加權） |
| `src/domain/arenaSystem.ts` | arena-pvp | 競技場排名系統 |
| `src/components/ArenaPanel.tsx` | arena-pvp | 競技場 UI 面板 |
| `src/components/CombatPowerHUD.tsx` | combat-power | 戰力 HUD 顯示元件 |
| `src/hooks/useCombatPower.ts` | combat-power | 戰力計算 hook |
| `src/domain/equipmentGacha.ts` | gacha §6 | 裝備抽卡 Domain 邏輯 |
| `src/components/AcquireToast.tsx` | item-acquire-toast | 獲得物品動畫提示 |
