...
| v2.24 | 2026-03-17 | DOT 雙係數系統（ATK + HP%）：(1) **統一 DOT 公式**——所有 DOT（中毒/灼燒/流血）統一為 `floor(source.ATK × value) + floor(min(target.maxHP, DOT_POISON_HP_CAP) × hpValue)`，取代原本毒傷HP-only、灼燒ATK-only 的分離公式；(2) **StatusEffect 新增 hpValue 欄位**——`hpValue?: number` 用於存儲 HP% 係數，applyStatusV2 疊加時同步累加；(3) **effect_templates 欄位慣例**——DOT 類效果的 `statusValue` 存 ATK 係數、`multiplier` 存 HP% 係數；(4) **14 筆 DOT 效果數據更新**——中毒效果新增 ATK 元件（statusValue=0.08~0.10），灼燒/流血效果新增 HP% 元件（multiplier=0.008~0.01）；(5) **4 筆舊版效果 category 修正**——EFF_DEBUFF_PAS_15_*/EFF_DEBUFF_SKL_* 的 category 從 debuff 改為 dot，確保描述面板正確顯示雙係數格式；(6) **SkillDescPanel 描述格式**——DOT 描述改為「攻擊×X% + 目標HP×Y%，HP傷害上限10萬」，單元件時隱藏另一方；(7) **Workers 補缺 cc/dot case**——executeSkill + executePassiveEffect 新增 case 'cc'/case 'dot' 處理（原本落入 default 不執行） |
