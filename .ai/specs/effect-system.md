...
| v2.24 | 2026-03-17 | DOT 雙係數系統（ATK + HP%）：(1) **統一 DOT 公式**——所有 DOT（中毒/灼燒/流血）統一為 `floor(source.ATK × value) + floor(min(target.maxHP, DOT_POISON_HP_CAP) × hpValue)`，取代原本毒傷HP-only、灼燒ATK-only 的分離公式；(2) **StatusEffect 新增 hpValue 欄位**——`hpValue?: number` 用於存儲 HP% 係數，applyStatusV2 疊加時同步累加；(3) **effect_templates 欄位慣例**——DOT 類效果的 `statusValue` 存 ATK 係數、`multiplier` 存 HP% 係數；(4) **4 筆舊版效果 category 修正**——EFF_DEBUFF_PAS_15_*/EFF_DEBUFF_SKL_* 的 category 從 debuff 改為 dot；(5) **SkillDescPanel 描述格式**——DOT 描述支援 ATK% 和 HP% 兩種顯示格式，HP% 時附帶「HP傷害上限10萬」；(6) **Workers 補缺 cc/dot case**——executeSkill + executePassiveEffect 新增 case 'cc'/case 'dot' 處理（原本落入 default 不執行） |
| v2.26 | 2026-03-18 | 技能描述面板 UI 改善：(1) **效果行增加 category header**——每個效果行頂部新增「分類 emoji + 分類中文名」+「效果名稱（灰色斜體）」，讓效果類型一目了然；(2) **附加標籤系統 EffectTags**——每個效果下方新增小圓標籤組，顯示目標（🎯 全體敵人）、觸發條件（受致命傷時）、觸發機率（50% 機率觸發）、每場上限（每場限 1 次）、最大疊層（最高 3 層）；(3) **類型標籤列**——技能名下方從純文字改為 badge 列，主動技/被動用不同顏色 badge，目標和鎖定各自獨立 badge；(4) **等級對比改為表格式逐效果展開**——從所有效果塞成一行改為每個效果各自一行橫向對比 Lv.1~5，相同效果自動跳過不顯示；(5) **視覺強化**——頂部彩色漸層裝飾線、icon 發光 drop-shadow、效果行 hover 高亮、backdrop-filter blur 遮罩、圓角 badge；(6) **DOT maxStacks 顯示**——dot 類型效果描述末尾新增「最高 N 層」標記 |
| v2.25 | 2026-03-18 | DOT 單係數獨立效果設計：(1) **設計理念**——每個 DOT 效果使用**單一**縮放基準（ATK% 或 HP%），同一種 DOT 類型（如中毒）可同時存在 ATK% 版本和 HP% 版本，作為獨立效果供不同技能搭配；(2) **14 筆 DOT 效果數據調整**——中毒 7 筆（2 ATK%：腐蝕蟲毒咬 0.08、瘟疫醫生被動 0.08；5 HP%：毒蕈師×2 0.02、瘟疫女王×2 0.02~0.03、瘟疫擴散 0.03）；灼燒 4 筆（3 ATK%：地獄灼燒 0.35、灼燒觸發 0.25、火焰爆裂 0.50；1 HP%：末日灼燒 0.03）；流血 3 筆（2 ATK%：箭雨 0.20、放血 0.40；1 HP%：瘟疫流血 0.02）；(3) **SQL 檔同步**——_insert_heroes_16_30.sql DOT 效果欄位對齊 D1 實際資料；(4) **引擎無需改動**——統一公式 `atkDmg + hpDmg` 在一邊為 NULL/0 時自然退化為單係數，SkillDescPanel 已能分別顯示「攻擊×X%」或「目標HP×Y%，HP傷害上限10萬」 |
