...
UI 已全面實作：2 個分類 Tab（道具/裝備）、排序功能、裝備圖鑑子介面、使用按鈕、金幣鉇石 Header。

## 依賴

- `.ai/specs/save-system.md` — `save_data.equipment` JSON 欄位（裝備獨立儲存）
- `.ai/specs/progression.md` — 消耗素材定義（經驗核心/碎片/職業石）、裝備模板制 v2
- `.ai/specs/gacha.md` — 星塵貨幣、抽卡產出
- `.ai/specs/stage-system.md` — 關卡掉落物
- `.ai/specs/hero-schema.md` — 英雄 HeroID 對應碎片
...
InventoryPanel mount
  ...
  │   └─ YES → loadInventory()
  │             ├─ loadItemDefinitions()    ← Workers API (有記憶體快取)
  │             ├─ Workers API: load-inventory  ← 取得 items（可堆疊道具）
  │             ├─ localStorage 合併：若 local 數量 > server 則取 local（樂觀寫入尚未同步）
  │             └─ 組成 InventoryState → 存 localStorage → 通知監聽者
  └─ NO  → 使用快取的 InventoryState
```

> **v2.0 變更**：背包載入不再包含 `equipment`。裝備資料由 `saveService` 的 `load-save` 獨立載入。
...
| # | 函式 | 說明 | API/Queue |
|---|------|------|-----------|
| 1 | `loadItemDefinitions(): Promise<Map<string, ItemDefinition>>` | 從 Workers 載入道具定義（有記憶體快取） | API `load-item-definitions` |
...
| 6 | `useItem(itemId, quantity, targetId?): Promise<{success, result?}>` | 使用道具（API 先調，成功後扣本地） | API `use-item` |
| 7 | `getItemQuantity(itemId): number` | 查詢某道具數量 | 純前端 |
| 8 | `filterItemsByCategory(category \| 'all'): InventoryItem[]` | 依分類篩選（'all' 回傳 qty>0） | 純前端 |
| 9 | `onInventoryChange(fn): () => void` | 訂閱背包變化，回傳 unsubscribe | 純前端 |
| 10 | `getInventoryState(): InventoryState \| null` | 取得當前記憶體中的背包狀態 | 純前端 |
| 11 | `addItemsLocally(items): void` | **不呼叫 API**，純前端樂觀新增（戰鬥掉落/抽卡碎片/信件獎勵） | 純前端 |
| 12 | `removeItemsLocally(items): void` | **不呼叫 API**，純前端樂觀扣除（升級/突破/升星素材扣除） | 純前端 |
| 13 | `clearInventoryCache(): void` | 清除記憶體 + localStorage 快取（登出時呼叫） | 純前端 |
...
- 若 `inventoryState` 為 `null`，自動建立最小狀態 `{ items: [], definitions: new Map() }`
- **不呼叫 API**，資料由 Workers 端在 `completeStage` / `gachaPull` 等 handler 中寫入 DB
- 用途 1：戰鬥勝利後的掉落物即時入帳
- 用途 2：抽卡重複英雄的碎片入帳
...
| action | handler | 說明 |
|--------|---------|------|
| `shop-buy` | `handleShopBuy_()` | 購買商品：扣貨幣 + 發放道具（Workers 驗證+冪等保護） |
...
Workers `generateChestRewards_(chestId, qty)` 依寶箱等級生成隨機獎勵：
...
| 點擊查看 | 點擊道具 → ItemDetail 彈窗（名稱、稀有度、描述、數量） |
...
```
通關結算 → addItemsLocally() 入帳掉落物 → Workers handler 同步寫入 DB
