...
- `.ai/specs/hero-schema.md` — HeroInstance 結構（HP, ATK, DEF, SPD, CritRate, CritDmg, stars）
- `.ai/specs/skill-system.md` — 被動技能星級解鎖
- `.ai/specs/save-system.md` — 存檔結構
- `.ai/specs/inventory.md` — 素材消耗與背包操作（碎片/職業石等）；EXP 已改為頂層資源（save_data.exp）
...
|------|--------|--------|--------|---------|----------|
| 武器 weapon | ATK | 30 | 50 | 80 | 120 |
| 護甲 armor | HP | 200 | 350 | 550 | 800 |
| 戒指 ring | DEF | 15 | 25 | 40 | 60 |
| 鞋子 boots | SPD | 5 | 8 | 12 | 18 |
...
### 套裝系統（8 套 × 2件/4件，不分稀有度）

同一英雄身上 4 件裝備中，**同 setId + 同 rarity** 的件數：
- ≥ 4 件 → 觸發 **4 件效果**（2 件 + 4 件效果皆生效）
- ≥ 2 件（< 4）→ 觸發 **2 件效果**
- 不同稀有度的同套裝**不計入**（例如 2 件 SSR 狂戰 + 1 件 SR 狂戰 → 只觸發 SSR 2 件）

> 注意：實際程式碼中套裝效果**不分稀有度**，所有稀有度共用同一組固定數值。

| 套裝 | 2 件效果 | 4 件效果 |
|------|---------|---------|
| berserker 狂戰士 | ATK +15% | CritDmg +20% |
| ironwall 鐵壁 | DEF +20% | HP +15% |
| gale 疾風 | SPD +15 | ATK +10% |
| vampire 吸血 | 吸血 12% | 吸血 +8%（額外） |
| critical 暴擊 | CritRate +12% | CritDmg +20% |
| lethal 致命 | CritDmg +25% | ATK +15% |
| vitality 生命 | HP +20% | DEF +15% |
| counter 反擊 | 反擊 20% | 反擊 +15%（額外） |

> 定義位於 `src/domain/progressionSystem.ts` `EQUIPMENT_SETS` 陣列。
> 套裝效果透過 `getActiveSetBonuses()` 計算，在 `getFinalStats()` 中套用至數值。
...
| 升星 | ⭐ 升星 | ✅ **已實作** — canStarUp 檢查 + 碎片費用 + Optimistic update + 屬性變化預覽 + 被動技能解鎖預覽（含技能說明文字） |
...
| v2.7 | 2026-03-07 | **升星 UI 改善**：①升星 Modal 新增被動技能說明顯示（`unlockPassive.description`），當星級會解鎖新被動時顯示技能 icon + 名稱 + 效果說明②CSS `.hd2-star-skill-desc`（color #bbb, 0.78rem） |
| v2.8 | 2026-03-15 | **Spec 校正 — 裝備系統對齊程式碼**：① 裝備主屬性修正：armor=HP（非 DEF）、ring=DEF（非 CritRate）② 基礎值全面更新（weapon 30/50/80/120、armor 200/350/550/800、ring 15/25/40/60、boots 5/8/12/18）③ 套裝效果簡化為不分稀有度的固定值（對齊 `EQUIPMENT_SETS` 實際程式碼）④ 鑽石裝備池十連 1,800→2,000、SR 25%→20%、N 27%→32%；金幣池 SR 10%→13%、N 53%→50% |