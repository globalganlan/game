...
> 產出日期：2026-03-02（第二次，平衡修正後）
> 測試範圍：完整遊戲流程（登入→教學→主選單→簽到→信箱→設定→關卡 1-1/1-2→英雄→背包→召喚→商店）
> 測試方法：Puppeteer headful（真實 Chrome 瀏覽器）+ WebGL 3D 渲染
> 測試腳本：`.ai/scripts/qa_full_playtest.mjs`
> 前端版本：本地 dev（Vite port 5176）+ Workers API（遠端 D1 已 re-seed）
...
> 對比前次（2025-07-14）：20 PASS / 0 FAIL / 3 WARN → **24 PASS / 0 FAIL / 0 WARN**
> 3 個 WARN 全部解決（1-2 勝利 → 召喚/商店解鎖）
| **FAIL** | 0 |
| **WARN** | 3（均因 1-2 敗北連鎖） |
| **TypeScript 編譯** | ✅ tsc --noEmit 零錯誤 |
| **Vite build** | ✅ 編譯成功 |
...
| 測試項 | 結果 | 說明 |
|--------|------|------|
| 頁面載入（3D Canvas 出現） | ✅ PASS | `<canvas>` 正常渲染 |
| 訪客登入 | ✅ PASS | 自動產生 guest token |
| 3D Canvas 存在 | ✅ PASS | WebGL context 正常 |

### Phase B — 新手教學

| 測試項 | 結果 | 說明 |
|--------|------|------|
| 教學跳過 / 主選單進入 | ✅ PASS | 偵測到 `[class*="main-menu"]` |
...
| 測試項 | 結果 | 說明 |
|--------|------|------|
| 進入關卡選擇 | ✅ PASS | 偵測到 stage-card 列表 |
| 點擊 1-1 關卡 | ✅ PASS | 透過 `button` + regex `/^1-1[^0-9]/` 精準定位 |
| 進入戰鬥準備（IDLE） | ✅ PASS | 偵測到 `.btn-bottom-center` |
| 點擊開始戰鬥 | ✅ PASS | — |
| 戰鬥完成 | ✅ PASS | 偵測到 `.battle-result-banner` |
| **1-1 勝利** | ✅ PASS | banner 文字含 `VICTORY` |

### Phase E — 關卡 1-2

| 測試項 | 結果 | 說明 |
|--------|------|------|
| 點擊「下一關 ▶」 | ✅ PASS | 1-1 勝利後直接進入 1-2 |
| 開始 1-2 戰鬥 | ✅ PASS | — |
| **1-2 結果** | ⚠️ WARN | **DEFEAT** — 初始 3 隻 Lv.1 英雄 vs 3 隻 Lv.3 敵人，戰力不足 |

### Phase F — 英雄 & 背包

| 測試項 | 結果 | 說明 |
|--------|------|------|
| 回到主選單 | ✅ PASS | 點擊「回大廳」正常回到主選單 |
| 英雄面板 | ✅ PASS | 1-1 通關後已解鎖 |
| 背包面板 | ✅ PASS | 1-1 通關後已解鎖 |

### Phase G — 召喚 & 商店

| 測試項 | 結果 | 說明 |
|--------|------|------|
| 召喚（抽卡） | ⚠️ WARN | 仍鎖定 — 需 1-2 通關解鎖 |
| 商店 | ⚠️ WARN | 仍鎖定 — 需 1-2 通關解鎖 |

### Phase H — Console 錯誤

| 測試項 | 結果 | 說明 |
|--------|------|------|
| 無致命 JS 錯誤 | ✅ PASS | 僅 3 筆 401 HTTP（guest API 預期行為） |

---

## 發現的 Bug（已修復）

### BUG-001：`saveService.ts` storyProgress notify 時序錯誤

- **嚴重度**：P1（功能性）
- **影響**：所有玩家通關後劇情進度不會推進
- **根因**：`updateStoryProgress()` 中 `enqueueSave()` 將 `storyProgress` 設為 JSON **字串**後呼叫 `notify()`，React 收到的是字串；隨後覆寫為正確的 **Object** 卻未再次呼叫 `notify()`
- **症狀**：`isFirstClear()` 計算 `progress.chapter` 時得到 `undefined`（字串沒有 `.chapter`），導致 `first = false`，進度永遠不會推進到下一關
- **修復**：在 Object 覆寫後補上 `notify()` 呼叫
- **檔案**：`src/services/saveService.ts`
- **狀態**：✅ 已修復 + tsc 通過 + spec 已更新（save-system.md v1.9）
...
| 項目 | 說明 |
|------|------|
| 1-2 關卡難度 | 新帳號僅 3 隻 Lv.1 英雄，面對 3 隻 Lv.3 敵人必敗。建議考慮：降低 1-2 敵人等級至 Lv.1~2，或給予新帳號更多初始資源 |
| 功能解鎖節奏 | 召喚/商店需 1-2 通關才解鎖，但 1-2 打不過就無法獲得新英雄/資源來增強戰力，形成死鎖 |

---

## 前次 QA 報告（2026-03-01 — 三大系統）

> 以下為前次 spec 合規性測試，保留供參考。
...
| `.ai/scripts/qa_gpu_e2e.mjs` | 9 項 E2E | 2 PASS / 7 WARN |
| `.ai/scripts/qa_three_systems.mjs` | 原始 E2E | 2 PASS / 8 WARN |
