...
2. **能用自動化就用自動化** — 優先使用 `.ai/scripts/qa_test_*.mjs` 腳本進行 E2E 測試
3. **確認功能可用後才能回覆使用者「完成」** — 不可以只報告 `tsc` + `vite build` 通過就結束
4. **測試流程必須覆蓋使用者會走的路徑** — 登入 → 到達功能畫面 → 操作 → 驗證結果
5. **截圖存證** — 將關鍵畫面截圖存入 `.ai/qa_screenshots/` 以供查閱
...
- [ ] DOT（灼燒/中毒）：每回合正確結算傷害
- [ ] 屬性剋制：fire vs water 等倍率正確（參照 `.ai/specs/element-system.md`）
...
> 以 `.ai/specs/` 目錄下的各 spec 文件為基準驗證。

- [ ] 傷害公式符合 `.ai/specs/damage-formula.md`
- [ ] 技能效果符合 `.ai/specs/skill-system.md`
- [ ] 被動觸發機制符合 `.ai/specs/core-combat.md` §6
- [ ] 能量系統符合 `.ai/specs/core-combat.md` §4（滿 1000 可放大招）
- [ ] 屬性剋制倍率符合 `.ai/specs/element-system.md`
- [ ] 被動解鎖星級符合 `.ai/specs/progression.md`（1★→1被動, 2★→2, 4★→3, 6★→4）
- [ ] 養成數值（升級、突破、裝備）計算正確
...
| QA 抽卡 E2E | `.ai/scripts/qa_test_gacha.mjs` | Puppeteer 自動：登入 → 召喚畫面 → 單抽/十連 → 驗證結果+鑽石 |
...
| Spec 文件 | 關鍵驗證點 |
|-----------|-----------|
| `.ai/specs/core-combat.md` | 戰鬥迴圈 / 能量系統 / Buff 持續 / 被動觸發 |
| `.ai/specs/damage-formula.md` | 傷害 = ATK × 技能倍率 × 剋制 × 暴擊 ÷ 防禦係數 |
| `.ai/specs/skill-system.md` | 主動技 1 個 / 被動技 4 個 / 星級解鎖 |
| `.ai/specs/element-system.md` | 6 屬性剋制表 / 剋制 +30% / 被剋 -30% |
| `.ai/specs/hero-schema.md` | 角色屬性欄位 / 稀有度 / 類型 |
| `.ai/specs/progression.md` | 升級 / 突破 / 裝備 / 星級被動解鎖 |
| `.ai/specs/gacha.md` | 抽卡機率 / 保底機制 |
| `.ai/specs/stage-system.md` | 關卡配置 / 敵方編成 |
| `.ai/specs/inventory.md` | 背包 / 物品 / 裝備穿脫 |
...
- **測試方式**：Puppeteer E2E（`.ai/scripts/qa_test_gacha.mjs`）
...
- 使用 `.ai/scripts/qa_gacha_stress.mjs` 執行 200 抽壓力測試
- 驗證項目：
...
- **refill race condition 防護**：
  ...
  - 測試工具：`.ai/scripts/qa_gacha_stress.mjs`
