...
- 容量顯示：`{items+equipment}/{capacity} 背包`（包含所有道具 + 裝備的總數）
...
1. **裝備穿戴**：
   - 裝備道具詳情顯示「裝備」按鈕（未穿戴時）
   - 點擊 → 彈出英雄選擇 popup（顯示實際英雄名稱）
   - 選擇英雄 → 呼叫 `equipItem(heroInstanceId, equipId)` → 樂觀更新 UI

2. **裝備卸下**：
   - 已穿戴裝備詳情顯示「卸下」按鈕 + 當前穿戴者英雄名
   - 點擊「卸下」→ 呼叫 `unequipItem(heroInstanceId, slot)` → 樂觀更新 UI
