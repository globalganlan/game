...
2. **`MenuScreen`** — 主選單子畫面（7 值），控制 MAIN_MENU 下的面板切換

另有頂層 `showGame` boolean 控制登入畫面與遊戲主體的切換。
所有畫面過渡均透過 **TransitionOverlay 過場幕** 實現無縫銜接。
...
1. 拉起過場幕（`setCurtainVisible(true)`）
2. 設定過場文字（`準備{displayName}...`）
3. `waitFrames(2)` 確保幕不透明
4. 設定 `stageMode`、`stageId`
5. 建立敵方陣型（`buildEnemySlotsFromStage`）
6. 恢復存檔陣型（`restoreFormationFromSave`）
7. `setMenuScreen('none')` + `setGameState('IDLE')`
8. `closeCurtain()` 收幕
...
### `retryBattle()`
1. 拉起過場幕（文字：`重新準備中...`）
2. 恢復戰前玩家陣容（HP 完全回復、死亡復活）
3. 重建同一關卡敵方
4. 清除所有戰鬥狀態（turn、log、popups、buffs、energy 等）
5. `setGameState('IDLE')` → 收幕
...
3. 清除戰鬥狀態
4. 收幕
5. 等待 `REPLAY_SCENE_SETTLE_MS`（400ms）
6. 以儲存的 `battleActionsRef` 呼叫 `runBattleLoop(savedActions)` 進行回放
...
GAMEOVER
  ...
  │   ├─ 「下一關 ▶」→ goNextStage() → [過場幕] → IDLE
  │   ├─ 「回放 ⏪」→ replayBattle() → [過場幕] → BATTLE（回放模式）
  │   ├─ 「戰鬥資訊 📊」→ 展開戰鬥統計面板
  │   └─ 「回大廳」→ backToLobby() → [過場幕] → MAIN_MENU
  │
  └─ defeat
      ├─ 顯示 DEFEAT banner
      ├─ 「重試」→ retryBattle() → [過場幕] → IDLE（同一關）
      ├─ 「回放 ⏪」→ replayBattle() → [過場幕] → BATTLE（回放模式）
      ├─ 「戰鬥資訊 📊」→ 展開戰鬥統計面板
      └─ 「回大廳」→ backToLobby() → [過場幕] → MAIN_MENU
```
