...
- **自動觸發信件**：新用戶註冊時自動寄送歡迎禮包（見 `specs/auth-system.md` v1.2）

## 依賴

- `specs/save-system.md` — playerId 驗證、`save_data.diamond` / `save_data.gold`
- `specs/inventory.md` — 非貨幣道具發放（`addItemsLocally`）
- `specs/optimistic-queue.md` — `fireOptimisticAsync` + `executeWithIdempotency_`
